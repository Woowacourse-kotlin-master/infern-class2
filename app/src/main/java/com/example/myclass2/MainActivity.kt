...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (i in 1..9) {
            // member1, member2 ... 의 ID를 가져오기
            val resId = resources.getIdentifier("member$i", "id", packageName)
            val memberView = findViewById<ImageView>(resId)

            memberView.setOnClickListener {
                val intent = Intent(this, ImageInsideActivity::class.java)
                intent.putExtra("data", i.toString())
